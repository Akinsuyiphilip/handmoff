				...
				<section className="w-full max-w-[1200px] px-4 py-10 text-hm-dark lg:px-0 lg:py-20">
						<div className="w-full relative -top-40">
							<Avatar className="size-40">
								...
								<AvatarFallback className="text-5xl font-heading">{initials(String(user?.name))}</AvatarFallback>
							</Avatar>
						</div>
						<div className="w-full flex flex-col gap-6">
							<div className="w-full flex items-center gap-4">
								<RiUserLine size={32} />
							<p className="text-xl capitalize font-medium">{user?.name}</p>
							</div>
							<div className="w-full flex items-center gap-4">
								<RiMailLine size={32} />
							<p className="text-xl lowercase font-medium">{user?.email}</p>
							</div>
							<div className="w-full flex items-center gap-4">
								<RiAtLine size={32} />
							<p className="text-xl lowercase font-medium">{user?.username}</p>
							</div>
						</div>
				</section>
			</main>
